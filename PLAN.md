...
## This PR — AWS laptop reach

`tofy run` on the applying machine must be able to use host `TOFY_*` URIs for postgres and redis. The account default security group does not admit a laptop, and `publicly_accessible` only on `Bind::All` was the wrong meaning of Localhost.
...
- `Bind::Localhost` means "not the whole internet" (applier `/32` only), not bind `127.0.0.1` on AWS. RDS `publicly_accessible` is set so the host URI is routable from that machine. `Bind::All` keeps public reach and opens ingress to `0.0.0.0/0`; the default example stays Localhost.
- Persist the `/32` in `.tofy/state.json` so destroy/plan stay stable. If the IP changed, plan shows an SG-rule update.
- Unit tests: tofy SG + `/32`, not `0.0.0.0/0` for Localhost; no `aws_vpc` resource; default-VPC data source stays; S3 unchanged; missing public-IP detection errors.
