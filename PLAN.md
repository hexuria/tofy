...
- `.size(Size::Small | Medium | Large)` and `.replicas(n)` (postgres stays at 1 locally)
- Host URIs vs `INTERNAL_*` DNS URIs; `tofy run` injects `TOFY_*`
- Secrets generated once, state mode `0600`, apply lock
- Postgres readiness wait; missing Docker emits artifacts and exits non-zero (not Applied)
- Required Docker CI: `ubuntu-latest`, `cargo test`, real apply, connect, destroy. **Fails if Docker is missing.**
...
- Keep the IR stable. Size tokens map to instance class; replicas stay attributes
...
- Importers into the same IR (including yaml as an importer only — not a write path)
